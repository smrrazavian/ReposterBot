...
from telegram.ext import *  #Bad Practice: #TODO: select exact function and module 
import os
...
def show_channels_command(update, context):
    if is_admin(update.message.chat.username):
        ...
        for i in range(len(Channels_list)):
            Channels_list_str += (
                Channels_list[i]["username"]
                + "\n\n\t\t\tID:"
                + str(Channels_list[i]["id"])
                + "\n\n\t\t\tPostInterval: "
                + str(Channels_list[i]["Interval"])
                + " Seconds\n\n\t\t\tPost Delay: "
                + str(Channels_list[i]["Repost_delay"])
                + "Minutes\n\n\n"
            )
        update.message.reply_text(
            f"You have added {len(Channels_list)} channels \n Your Channels list:\n {Channels_list_str}"
        )
        update.message.reply_text(
        ...
        if args:
            ...
            for i in range(len(Channels_list)):
                ...
                if Channels_list[i]["username"] == channel_username:
                    ...
                    update.message.reply_text(
                        f"Channel {channel_username} has been removed"
                    )
                    break
            with open("posts.json", "w") as file:
                file.write(json.dumps(posts) + "\n")
...
def edit_post_keyword_command(update, context):
    if is_admin(update.message.chat.username):
        ...
        if args:
            ...
            update.message.reply_text(
                f"The Post Keyword has been updated to: {keyword}"
            )
        else:
    ...
    if is_admin(update.message.chat.username):
        ...
        if args:
            ...
            with open("config.ini", "w") as configfile:
                config.write(configfile)
            update.message.reply_text(
                f"The agent bot token has been updated to: \n {bot_token}"
            )
        else:
    ...
    if Bot_Status == "True":
        for i in range(0, len(data)):
            ...
            if data[i]["message_id"][username] != "":
                try:
                    bot.delete_message(
                        chat_id=username, message_id=data[i]["message_id"][username]
                    )
                except:
                    pass
            # try:
        ...
        if args:
            ...
            try:
                if status == str(1):
                    ...
                    update.message.reply_text(
                        "Invalid Input for chaning the state of the Bot.(please enter between 0 and 1)"
                    )
            except:
                print(error)
                update.message.reply_text(
                    "Invalid Input for chaning the state of the Bot.(please enter between 0 and 1)شسبشسبش"
                )
        else:
...
def handle_response(update, text) -> str:
    ...
    if is_admin(update.message.chat.username):
        ...
        if Post_keyword.lower() in text.lower() or Post_keyword.upper() in text.upper():
            ...
            posts = json.load(f)
            last_id += 1
            data = {"fid": str(last_id), "message": text, "message_id": a}
                    ...
                    for j in range(len(posts[i]["message_id"])):
                        ...
                        if posts[i]["message_id"][username] != "":
                            try:
                                ...
                                bot.delete_message(
                                    chat_id=username,
                                    message_id=posts[i]["message_id"][username],
                                )
                            except:
                                pass
                    posts.pop(i)
